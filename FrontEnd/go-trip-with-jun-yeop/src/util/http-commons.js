...
const {
  ...
  VITE_SEARCH_URL,
  BACK_END_API_URL,
  VITE_KAKAOMOBILITY_SERVICE_KEY,
} = import.meta.env;

// 백엔드
function localAxios() {
  let tmp = BACK_END_API_URL;
  tmp = VITE_VUE_API_URL;
  let baseURL = "http://localhost:8080";
  console.log(tmp, baseURL);
...
// 관광지 정보_키워드 O
function attractionKeyWordAxios() {
  const instance = axios.create({
    baseURL: VITE_SEARCH_URL,
    headers: {
      "Content-Type": "application/json;charset=utf-8",
    },
  });
...
//다중 경로 길찾기
function multiPathAxios() {
  const instance = axios.create({
    baseURL: "https://apis-navi.kakaomobility.com/v1/directions",
    headers: {
      Authorization: `KakaoAK ${VITE_KAKAOMOBILITY_SERVICE_KEY}`,
      "Content-Type": "application/json;charset=utf-8",
    },
  });
  return instance;
